...
/* ============================================================
   COULEURS / THÈMES
   ============================================================ */
const TKEY = "loto64-theme-v2"; // v2 : couleur de base pastel
const DEFAULT_THEME = {
  bg: "#0f1437", bg2: "#161c4d", card: "#1d2560", card2: "#232c75",
  ink: "#f5f7ff", inkSoft: "#b9c0e8", accent: "#ffd23f", accent2: "#ff8a3d"
};
// couleur de base de l'application (pastel)
const BASE_THEME = { bg: "#e7d9ff", bg2: "#efe6ff", card: "#ffffff", card2: "#f3ecff", ink: "#3c2b57", inkSoft: "#6f5f8c", accent: "#b15bff", accent2: "#ff8ac4" };
const THEMES = [
...
function applyTheme(v) {
  ...
  markActiveSwatch();
  try { localStorage.setItem(TKEY, JSON.stringify(v)); } catch (e) { /* ignore */ }
}
...
function setupTheme() {
  buildThemeSwatches();
  let saved = null;
  try { saved = JSON.parse(localStorage.getItem(TKEY)); } catch (e) { /* ignore */ }
  applyTheme(saved && saved.bg ? saved : BASE_THEME);
  ...
  el.themeReset.addEventListener("click", () => applyTheme(BASE_THEME));
...
function setupLock() {
  ...
  const tryUnlock = () => {
    ...
    if (code === normCode(OWNER_PWD)) { doUnlock(codeFor("proprietaire")); }  // mot de passe propriétaire accepté ici aussi
    else if (codeValid(code)) { doUnlock(code); }
